...
int DIR_L_F = 13;  // Left Direction pin that will indicate forewards movement
                   // (1 for forewards, 0 for backwards).
int DIR_L_B = 12;  // Left Direction pin that will indicate backwards movement
                   // (1 for backwards, 0 for forewards).
int PWM_L = 11;    // Speed controll pin. *** This pin must be plugged into an
                   // output pin on the arduino that is labled PWM ***.

int DIR_R_F = 4;  // Right Direction pin that will indicate forewards movement
                  // (1 for forewards, 0 for backwards)
int DIR_R_B = 2;  // Left Direction pin that will indicate backwards movement (1
                  // for backwards, 0 for forewards).
int PWM_R = 3;    // Speed controll pin. *** This pin must be plugged into an
                  // output pin on the arduino that is labled PWM ***.
...
void GetBTCommand();  // Outputs a list containing the raw controller inputs.
...
void SimpleMapInput();  // A pre write function That takes in TransatedCommand
                        ...
                        // and right motors.
void ExecuteCommand_L298N();  // Takes in a 4 element list (Left Direction,
...
void setup() {
  Serial.begin(9600);  // Initializing the Serial Port to Baud rate 9600. You
                       // can change this to be faster if you wish.
...
void UnicaTranslateCommand(float* ans, String Input) {
  float angle = float(
      Input.substring(0, 3).toInt());  // First get the abc numbers out of the
                                       // command and then convert to integer.
  float power = float(
      Input.substring(3, 6).toInt() *
      2.55);  // Next get the def numbers out of the command and then convert to
              // integer. Finally convert this 0-100 value into a 0-255 value.
  int button = float(
      Input.substring(6).toInt());  // Finally get the g number out of the
                                    // command and then convert to integer.
  ans[0] = angle;
  ans[1] = power;
  for (int i = 2; i < 7; i++) {
